...
async function showSecurityPolicy(): Promise<void> {
  ...
  console.log(
    `  Cross-engine operations: ${policy.allowCrossEngineOperations ? chalk.green('Allowed') : chalk.red('Blocked')}`,
  );
  console.log(
    `  Network isolation: ${policy.enableNetworkIsolation ? chalk.green('Enabled') : chalk.yellow('Disabled')}`,
  );
  ...
  console.log(chalk.bold('\nAudit:'));
  console.log(
    `  Logging: ${policy.auditOperations ? chalk.green('Enabled') : chalk.red('Disabled')}`,
  );
  ...
  try {
    ...
    recentLogs.forEach((logLine) => {
      try {
        ...
        console.log(
          `${status} ${chalk.gray(timestamp)} ${chalk.cyan(log.operation)} ${log.source}${target}`,
        );
...
async function handleSecurity(options: SecurityOptions): Promise<void> {
  ...
  const scoreColor =
    securityScore >= 80 ? chalk.green : securityScore >= 60 ? chalk.yellow : chalk.red;

  console.log(`  Security Score: ${scoreColor(securityScore)}/100`);
  console.log(
    `  Audit Logging: ${policy.auditOperations ? chalk.green('✅ Enabled') : chalk.red('❌ Disabled')}`,
  );
  console.log(
    `  Network Isolation: ${policy.enableNetworkIsolation ? chalk.green('✅ Enabled') : chalk.yellow('⚠️  Optional')}`,
  );
